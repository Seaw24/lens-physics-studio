...
export function createStudioRouter(studio: StudioService) {
  ...
  router.use((req, res, next) => {
    ...
    try {
      studio.discovery.auth.requireController(req);
    } catch (error) {
      return next(error);
    }
    next();
  ...
  router.get(
    ...
    asyncRoute(async (_req, res) => {
      res.json(await studio.day());
    }),
  );
  router.get(
    "/events/:id",
    asyncRoute(async (req, res) => {
      res.json(await studio.event(eventId(req)));
    }),
  );
  router.post(
    "/events/:id/generate",
    ...
    asyncRoute(async (req, res) => {
      ...
      res.json(await studio.generate(eventId(req), force));
    }),
  );
  router.get(
    "/events/:id/poster",
    asyncRoute(async (req, res) => {
      const file = await studio.poster(eventId(req));
